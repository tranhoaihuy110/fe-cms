...
export const FormModal = <T extends Record<string, any>>({
  ...
  const handleSelectChange = (
    s: React.ChangeEvent<HTMLSelectElement>,
    field: IFormField<T>
  ) => {
    const { name, value, selectedOptions } = s.target;
    const newValue = field.selectOptions?.multiple
  ...
  const handleDateChange = (date: Date | null, field: IFormField<T>) => {
    setFormData((prev) => ({
      ...prev,
      [field.name]: date,
    }));
    field.onChangeDate?.(date);
  };
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-3">
              {fields.map((field) => (
                ...
                >
                  ...
                  {field.type === "custom" ? (
                    field.renderValue ? (
                      <div className="mt-1">{field.renderValue()}</div>
                    ) : (
                  ...
                  : field.type === "select" ? (
                    <div className="relative">
                      ...
                      {field.clearable && formData[field.name] && !isDetailMode && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() => handleClear(field)}
                          className="ml-2 absolute right-0 top-1/2 transform -translate-y-1/2"
                          disabled={loading || field.disabled}
                        >
                          <svg
                            className="h-4 w-4"
                    ...
                    <AceEditor
                      ...
                      setOptions={{
                        ...
                        enableSnippets:
                          field.aceOptions?.enableSnippets ?? true,
                        showLineNumbers:
                    ...
                    <div className="relative">
                      <DatePicker
                        ...
                        showTimeSelect={
                          field.datePickerOptions?.showTimeSelect ?? true
                        }
                        timeFormat={
                          field.datePickerOptions?.timeFormat || "HH:mm:ss"
                        }
                        timeIntervals={
                          field.datePickerOptions?.timeIntervals || 15
                        }
                        dateFormat={
                          field.datePickerOptions?.dateFormat ||
                          "yyyy-MM-dd HH:mm:ss"
                        }
                        minDate={field.datePickerOptions?.minDate}
                        maxDate={field.datePickerOptions?.maxDate}
                        className="w-full rounded-md border border-gray-300 px-6 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        disabled={loading || isDetailMode || field.disabled}
                      />
                      {field.clearable &&
                        formData[field.name] &&
                        ...
                        >
                          <div className="flex-1">
                            <Input
                              ...
                              disabled={
                                loading || isDetailMode || field.disabled
                              }
                            />
                          </div>
                          <div className="w-20">
                            ...
                            <div className="ml-2">
                              {index ===
                                ...
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    addUrlListItem(String(field.name))
                                  }
                                  disabled={loading}
                                  className="mr-2"
                                >
