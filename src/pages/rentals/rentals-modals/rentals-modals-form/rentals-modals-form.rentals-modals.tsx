...
export const RentalsFormModal: React.FC<IRentalsFormModalProps> = ({
          ...
          try {
            ...
            if (results && results.length > 0) {
              ...
              const newFormData = {
                ...
                tenant_name:
                  fetchedConfig.tenant_name || config.tenant_name || "",
                tenant_phone:
                  fetchedConfig.tenant_phone || config.tenant_phone || "",
                tenant_email:
                  fetchedConfig.tenant_email || config.tenant_email || "",
                rental_start_date:
                  fetchedConfig.rental_start_date ||
                  config.rental_start_date ||
                  "",
                rental_end_date:
                  fetchedConfig.rental_end_date || config.rental_end_date || "",
                rental_price:
                  fetchedConfig.rental_price || config.rental_price || "",
                deposit: fetchedConfig.deposit || config.deposit || "",
  ...
  const fields: IFormField<IRentalsGetApi>[] = [
    ...
    } as const,
    {
      ...
      onChangeDate: (date) => handleDateChange(date, "rental_start_date"),
    } as const,
    {
      ...
      onChangeDate: (date) => handleDateChange(date, "rental_end_date"),
    } as const,
    {
      name: "rental_price",
      label: "Rental Price",
