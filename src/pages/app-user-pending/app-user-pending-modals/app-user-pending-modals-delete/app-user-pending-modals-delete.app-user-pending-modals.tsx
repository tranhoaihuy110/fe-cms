...
> = ({ isOpen, onClose, onConfirm, config }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      onClose();
      toast.success(`App User Pending with user name "${config.username}" deleted successfully!`);
    } catch (error) {
      const err = error as { message: string; error?: string };
      const errorMessage =
        err.message || err.error || `Failed to delete App User Pending with key "${config.username}".`;
      ...
      <div className="relative w-full max-w-[500px] rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8">
        <div className="px-2">
          <h4 className="mb-2 text-xl font-semibold text-gray-800 dark:text-white/90">
            Delete App User Pending
          </h4>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
            Are you sure you want to delete the App User Pending with User Name{" "}
