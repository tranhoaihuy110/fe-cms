...
> = ({ isOpen, onClose, onSubmit, mode, config}) => {
  ...
  const [typeSuggestions, setTypeSuggestions] = useState<ILeadPropertyFloorsGetApi[]>([]);
  ...
  const [showPropertyEmailSuggestions, setShowPropertyEmailSuggestions] = useState(false);
          ...
          try {
            ...
            if (results && results.length > 0) {
              ...
              const newFormData = {
                ...
                lead_property_id:
                  fetchedConfig.lead_property_id || config.lead_property_id || "",
                floor_id: fetchedConfig.floor_id || config.floor_id || "",
                floor_name:
                  fetchedConfig.floor_name ||
                  config.floor_name ||
                  "",
