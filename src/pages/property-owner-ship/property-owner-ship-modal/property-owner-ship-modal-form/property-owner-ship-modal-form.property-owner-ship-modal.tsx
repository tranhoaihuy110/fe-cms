...
> = ({ isOpen, onClose, onSubmit, mode, config, children = "" }) => {
  ...
  const handleDateChange = (date: Date | null, fieldName: keyof IPropertyOwnerShipGetApi) => {
  setFormData((prev) => ({
    ...prev,
  ...
  const fields: IFormField<IPropertyOwnerShipGetApi>[] = [
    ...
    } as const,
    {
      ...
      onChangeDate: (date) => handleDateChange(date, "start_date"),
    } as const,
    {
      name: "end_date",
      label: "End Date",
      ...
      onChangeDate: (date) => handleDateChange(date, "end_date"),
    } as const,
  ];
